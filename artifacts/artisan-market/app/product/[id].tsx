...
export default function ProductDetailScreen() {
  ...
  const { getProduct, addReceipt } = useArtisan();
  ...
  const buyProduct = async () => {
    if (!product.shopifyVariantId) {
      Alert.alert('Checkout is being connected', 'This product needs to be matched to an active Shopify product before buyers can pay for it.');
      return;
    }
    try {
      const baseUrl = process.env.EXPO_PUBLIC_DOMAIN ? `https://${process.env.EXPO_PUBLIC_DOMAIN}` : '';
      const response = await fetch(`${baseUrl}/api/shopify/checkout`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ variantId: product.shopifyVariantId, quantity: 1 }) });
      const checkout = (await response.json()) as { checkoutUrl?: string; message?: string };
      if (!response.ok || !checkout.checkoutUrl) throw new Error(checkout.message ?? 'Checkout could not be created.');
      await addReceipt({ productName: product.name, amount: product.price, status: 'Payment started', checkoutUrl: checkout.checkoutUrl });
    ...
    <View style={[styles.screen, { backgroundColor: colors.background }]}>
      <ScrollView contentContainerStyle={{ paddingBottom: insets.bottom + 25 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.body}><View style={styles.titleRow}><View style={{ flex: 1 }}><Text style={[styles.craft, { color: colors.primary }]}>{product.craft.toUpperCase()}</Text><Text style={[styles.title, { color: colors.foreground }]}>{product.name}</Text></View><Text style={[styles.price, { color: colors.primary }]}>₹{product.price.toLocaleString('en-IN')}</Text></View><Text style={[styles.material, { color: colors.mutedForeground }]}>{product.material}</Text><View style={[styles.divider, { backgroundColor: colors.border }]} />
          ...
          <Pressable style={[styles.buyButton, { backgroundColor: colors.primary }]} onPress={() => { void buyProduct(); }}><Ionicons name="card-outline" size={18} color={colors.primaryForeground} /><Text style={[styles.shareText, { color: colors.primaryForeground }]}>Buy securely with Shopify</Text></Pressable>
          <Pressable style={[styles.shareButton, { borderColor: colors.border }]}><Feather name="share-2" size={17} color={colors.primary} /><Text style={[styles.shareText, { color: colors.primary }]}>Share listing</Text></Pressable>
        </View>
      </ScrollView>
    </View>
  );
